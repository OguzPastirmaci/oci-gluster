...
create_headnode()
{
  ...
  for i in `seq 1 $server_nodes`; do
    BV=`oci bv volume create $INFO --display-name "gluster-block-$PRE-$i" --size-in-gbs $BLKSIZE_GB --wait-for-state AVAILABLE | jq -r '.data.id'`;
    masterID=`oci compute instance launch $INFO --shape "$gluster_server_shape" --display-name "glusterfs-server$i" --image-id $OS --subnet-id $S --private-ip 10.0.$subnet.1$i --wait-for-state RUNNING --user-data-file scripts/gluster_configure.sh --ssh-authorized-keys-file $PRE.key.pub | jq -r '.data.id'`;
