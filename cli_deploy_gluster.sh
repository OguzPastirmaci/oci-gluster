...
create_headnode()
{
  ...
  for i in `seq 1 $server_nodes`; do
    ...
    masterID=`oci compute instance launch $INFO --shape "$SIZE" --display-name "glusterfs-server$i" --image-id $OS --subnet-id $S --private-ip 10.0.$subnet.2 --wait-for-state RUNNING --user-data-file scripts/gluster_configure.sh --ssh-authorized-keys-file $PRE.key.pub | jq -r '.data.id'`;
