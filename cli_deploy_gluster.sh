...
create_headnode()
{
  ...
  for i in `seq $server_nodes -1 1`; do
    ...
    masterID=`oci compute instance launch $INFO --shape "$SIZE" --compartment-id $compartment_id --display-name "gluster-server-$PRE-$i" --image-id $OS --subnet-id $S --private-ip 10.0.$subnet.1$i --wait-for-state RUNNING --user-data-file scripts/gluster_configure.sh --ssh-authorized-keys-file $PRE.key.pub | jq -r '.data.id'`
