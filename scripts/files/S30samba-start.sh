#!/bin/bash
# Name: S30samba-start.sh
# Modified By: Chuck Gilbert <chuck.gilbert@oracle.com>
# NOTE: This modification is to ensure proper SMB locking is added
#  for correct mounts on Windows Servers.
#
...
function add_samba_share () {
        ...
        STRING+="glusterfs:loglevel = 0\n"
        STRING+="path = /\n"
        STRING+="read only = no\n"
        STRING+="guest ok = yes\n"
        STRING+="kernel share modes = no\n"
        STRING+="kernel oplocks = no\n"
        STRING+="map archive = no\n"
